...
sheetType='Z';
distributionSelect='normal';% spikes must have poission or binomial
...
data = readmatrix([pathIn,fileName,],'Sheet',sheetType);
...
parfor neuron1 = 1:numCols
    ...
    for k = 1:K
        ...
        if STANDARDIZE_FOR_CV
            ...
            XtrZ = (X(tr,:) - mu) ./ sig;
            XteZ = (X(te,:) - mu) ./ sig;
            mdl_k   = fitglm(XtrZ, y(tr), 'Distribution',distributionSelect, 'Intercept', true);
    ...
    if STANDARDIZE_FOR_CV
        ...
        XtrZ = (X(tr,:) - muH) ./ sigH;
        XteZ = (X(te,:) - muH) ./ sigH;
        mdl_ho  = fitglm(XtrZ, y(tr), 'Distribution',distributionSelect, 'Intercept', true);
